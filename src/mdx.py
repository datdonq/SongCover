...
def load_mdx_model(model_params,model_path):
    ...
    return mdx_sess,model
# def run_mdx(mdx_sess,model, output_dir, filename, exclude_main=False, exclude_inversion=False, suffix=None, invert_suffix=None, denoise=False, keep_orig=True, m_threads=2):
...
def run_mdx(mdx, filename, m_threads=2):
    mdx_sess=mdx[0]
    model=mdx[1]
    wave, sr = librosa.load(filename, mono=False, sr=44100)
    # normalizing input wave gives better output
    ...
    # return to previous peak
    wave_processed *= peak
    return wave_processed.T, (-wave_processed.T * model.compensation) + wave.T, sr
